...
from pysph.base.gpu_utils import DeviceHelper, DeviceArray
...
class TestDeviceArray(TestCase):
    def make_dev_array(self, n=16):
        ...
        dev_array.fill(0)
        dev_array.array[0] = np.asarray(1, dtype=np.int32)
        return dev_array
    ...
    def test_extend(self):
        # Given
        dev_array = self.make_dev_array()
        cl_array = 2 + gm.zeros(64, dtype=np.int32)
    ...
    def test_remove(self):
        ...
        orig_array = gm.arange(0, 16, 1, dtype=np.int32)
        dev_array.set_data(orig_array)
        indices = gm.arange(0, 8, 1, dtype=np.int32)
    ...
    def test_copy_values(self):
        ...
        dest = gm.empty( 8, dtype=np.int32)
        indices = gm.arange(0, 8, 1, dtype=np.int32)
...
class TestDeviceHelper(TestCase):
    ...
    def test_pull_correctly_sets_values_with_args(self):
        ...
        h.rho[0] = np.asarray(1.0, dtype=h.rho.dtype)
        h.tag[:] = gm.ones_like(h.tag)
        h.pull('x', 'rho', 'tag')
    ...
    def test_pull_correctly_sets_values_with_no_args(self):
        ...
        # When
        pa.set_device_helper(h)
        h.x[:] = gm.ones_like(h.x)
        h.rho[:] = gm.ones_like(h.x)
        h.m[:] = gm.ones_like(h.x)
    ...
    def test_align(self):
        ...
        indices = gm.arange(4, -1, -1, dtype=np.int32)
